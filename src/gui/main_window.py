...
class Ui_mainWindow(QObject):
    def __init__(self):
        ...
        self.excluded_keywords = (
            "box case break cgc cu ga ags lc tcm hga"
    ...
    def setupUi(self):
        ...
        if loaded_file != {}:
            ...
            new_default_path = self.settingsWindow.set_default_save_from_settings(
                loaded_file, self.settings_path
            )
        ...
        self.mainWindow.setToolButtonStyle(QtCore.Qt.ToolButtonStyle.ToolButtonIconOnly)
        ...
        self.lblService.setStyleSheet(
            f"font-size: 12pt; color: {self.current_theme['foreground']}; text-align: center;"
        )
        self.lblService.setAlignment(
            QtCore.Qt.AlignmentFlag.AlignLeading
            | QtCore.Qt.AlignmentFlag.AlignLeft
            | QtCore.Qt.AlignmentFlag.AlignVCenter
        )
        ...
        self.btnSearch.setLayoutDirection(QtCore.Qt.LayoutDirection.LeftToRight)
        ...
        def default_save_content():
            if self.default_response == {}:
                ...
                self.utils.dict_to_txt(
                    self.default_response, self.default_save, self.txtSearch.text()
                )
    ...
    def handle_api_error(self):
        if Ebay.code != 200:
            ...
            if Ebay.code == 429:
                show_dialog(
                    "success", f"{Ebay.code}: You've exceeded your monthly call limit"
                )
            elif Ebay.code == 401:
                show_dialog(
                    "error",
                    f"{Ebay.code}: API key not found. Please enter your API key to continue.",
                )
            return

    def render_images(
        self, response, imgHigh, imgLow, descendingResults, ascendingResults
    ):
        ...
        self.lblCardHighest.setPixmap(
            imgHigh.transformed(QTransform().rotate(-90))
            if pmWidthHighest > pmHeightHighest
            else imgHigh
        )
        self.txtInfoHighest.setHtml(descendingResults)
        pmWidthLowest, pmHeightLowest = imgLow.width(), imgLow.height()
        self.lblCardLowest.setPixmap(
            imgLow.transformed(QTransform().rotate(-90))
            if pmWidthLowest > pmHeightLowest
            else imgLow
        )
    ...
    def search(self):
        ...
        if imgHigh is not None and imgLow is not None:
            self.render_images(
                response, imgHigh, imgLow, descendingResults, ascendingResults
            )
    ...
    def retranslateUi(self, mainWindow):
        ...
        self.tabMain.setTabText(
            self.tabMain.indexOf(self.tabHighest), _translate("mainWindow", "Highest")
        )
        self.tabMain.setTabToolTip(
            self.tabMain.indexOf(self.tabHighest),
            _translate("mainWindow", "Highest Sold Price"),
        )
        self.tabMain.setTabText(
            self.tabMain.indexOf(self.tabLowest), _translate("mainWindow", "Lowest")
        )
        self.tabMain.setTabToolTip(
            self.tabMain.indexOf(self.tabLowest),
            _translate("mainWindow", "Lowest Sold Price"),
        )
        self.tabMain.setTabText(
            self.tabMain.indexOf(self.tabHistory), _translate("mainWindow", "History")
        )
        self.tabMain.setTabToolTip(
            self.tabMain.indexOf(self.tabHistory),
            _translate("mainWindow", "Sales History"),
        )
